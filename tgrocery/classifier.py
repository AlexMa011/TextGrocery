...
class GroceryTextModel(object):
    def __init__(self, text_converter=None, model=None):
        if text_converter is not None and isinstance(text_converter,
                                                     GroceryTextConverter):
    ...
    def __str__(self):
        return 'TextModel instance ({0}, {1})'.format(self.text_converter,
                                                      self.svm_model)

    def get_labels(self):
        return [
            self.text_converter.get_class_name(k)
            for k in self.svm_model.get_labels()
        ]
    ...
    def save(self, model_name, force=False):
        ...
        try:
            ...
            raise OSError(
                e, 'Please use force option to overwrite the existing files.')
    ...
    def predict_text(self, text):
        if self.svm_model is None:
            raise Exception(
                'This model is not usable because svm model is not given')
        ...
        return GroceryPredictResult(predicted_y=y,
                                    dec_values=dec[:self.svm_model.nr_class],
                                    labels=labels)
